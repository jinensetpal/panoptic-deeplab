...
from src.model import convolutions
...
class ASPP(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        if not aspp_use_only_1x1_proj_conv and len(atrous_rates) != 3:
            raise ValueError('The ASPP layers need exactly 3 atrous rates, but %d were given' % len(atrous_rates))
    ...
    def set_pool_size(self, pool_size):
        ...
        """
        if not self._aspp_use_only_1x1_proj_conv: self._aspp_pool.set_pool_size(pool_size)

    def get_pool_size(self):
        if not self._aspp_use_only_1x1_proj_conv: return self._aspp_pool.get_pool_size()
    ...
    def call(self, input_tensor, training=False):
        ...
        Returns:
          The output tensor.
        """
        if self._aspp_use_only_1x1_proj_conv: x = self._proj_conv_bn_act(input_tensor, training=training)
        else:
